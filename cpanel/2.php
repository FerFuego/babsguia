				...
				<div class="col-xs-12 col-sm-4">
					...
					<?php
					 if(isset($_SESSION['logino'])):
							...
							<label>Apto</label><br><?php if(is_numeric($id_tour)){print'<div class="alert alert-info">Si modifica un registro, deberá volver a tildar los campos correspondientes.</div>';}else{}?>
							...
							<?php if(is_numeric($id_tour)){print'<div class="alert alert-info">Si modifica un registro, deberá cambiar esto: (galerias/tours/nombrecarpeta/) y borrar hasta dejar solo (nombrecarpeta).</div>';}else{}?>
							...
							<?php if(is_numeric($id_tour)){print'<div class="alert alert-info">Si modifica un registro, no será necesario volver a cargar las imágenes.</div>';}else{}?>
							...
							<input type="submit" name="submit" class="btn btn-primary" value ="<?php if(is_numeric($id_tour)) print "Modificar - Update"; else print "Cargar - Insert";?>">
							</div>
							</form>
						<?php
						//***** NOVEDADES CODES *****//
						...
						if(isset($_POST['silla'])){$datos->set10($_POST['silla']);}else{$datos->set10("off");}
						if(isset($_POST['down'])){$datos->set11($_POST['down']);}else{$datos->set11("off");}
						if(isset($_POST['hipoacustico'])){$datos->set12($_POST['hipoacustico']);}else{$datos->set12("off");}
						if(isset($_POST['lenguaje'])){$datos->set13($_POST['lenguaje']);}else{$datos->set13("off");}
						if(isset($_POST['novidente'])){$datos->set14($_POST['novidente']);}else{$datos->set14("off");}
						if(isset($_POST['universitario'])){$datos->set15($_POST['universitario']);}else{$datos->set15("off");}		
