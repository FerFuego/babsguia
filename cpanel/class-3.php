...
class Servicio{
	...
	function Servicio($nro=0){
		if($nro!=0){
			$obj_prod = new sQuery();
			$resultado = $obj_prod->executeQuery("SELECT * FROM servicios WHERE id_servicios=$nro");
			$fila=mysql_fetch_assoc($resultado);
			$this->id_servicios=$fila['id_servicios'];
	...
	function get(){
		$obj_new = new sQuery();
		$resultado = $obj_new->executeQuery("SELECT * FROM servicios ORDER BY id_servicios ASC");
		return $resultado;
	}
	...
	function getrowID($var){
		$obj_new = new sQuery();
		$resultado = $obj_new->executeQuery("SELECT * FROM servicios WHERE id_servicios='$var'");
		return $resultado;
	}

	function getID(){ return $this->id_servicios; }
	...
	function insert(){
			$obj_new=new sQuery();
			$query="insert into servicios(atributo1, atributo2, atributo3, atributo4, atributo5, atributo6, atributo7, atributo8, atributo9)values('$this->atributo1','$this->atributo2','$this->atributo3','$this->atributo4','$this->atributo5','$this->atributo6','$this->atributo7','$this->atributo8','$this->atributo9')";
			$obj_new->executeQuery($query); 
	}	

	function update(){
			$obj_new=new sQuery();
			$query="update servicios set atributo1='$this->atributo1', atributo2='$this->atributo2', atributo3='$this->atributo3', atributo4='$this->atributo4', atributo5='$this->atributo5', atributo6='$this->atributo6', atributo7='$this->atributo7', atributo8='$this->atributo8', atributo9='$this->atributo9' where id_servicios = $this->id_servicios";
			$obj_new->executeQuery($query);
	}

	function delete($val){
			$obj_new=new sQuery();
			$query="delete from servicios where id_servicios=$val";
			$obj_new->executeQuery($query);
	}
