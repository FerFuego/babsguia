				...
				<div class="col-xs-12 col-sm-6">
					<?php
					 if(isset($_SESSION['logino'])):
							...
							<input type="hidden" name="id_noticia" class="form-control" value="<?php print $_GET['me'] ?>">
							...
							<div class="form-group">
							<br>
							<input type="submit" name="submit" class="btn btn-primary" onClick = "Ok();" value ="<?php if(is_numeric($id_noticia)) print "Modificar - Update"; else print "Cargar - Insert";?>">
							</div>
							</form>
						<?php
						//***** PRENSA CODES *****//
						if (isset($_POST['submit'])&&isset($_POST['id_noticia'])&&!is_numeric($_POST['id_noticia'])) {
							...
							if ($archivoimg) { $datos->set3($archivoimg); };
							if ($archivoimg1) { $datos->set7($archivoimg1); };
							if ($archivoimg2) { $datos->set8($archivoimg2); };
							if ($archivoimg3) { $datos->set9($archivoimg3); };	
							$datos->insert(); 
							$close= $datos->closeconexion();
						...
						if (isset($_POST['submit'])&&isset($_POST['id_noticia'])&&is_numeric($_POST['id_noticia'])) {
							...
							$datos->set5(0);	
							$datos->set6($_POST['lenguaje']);	
							if ($archivoimg || $_POST['control_img']) { $datos->set3($archivoimg); };
							if ($archivoimg1 || $_POST['control_img1']) { $datos->set7($archivoimg1); };
							if ($archivoimg2 || $_POST['control_img2']) { $datos->set8($archivoimg2); };
							if ($archivoimg3 || $_POST['control_img3']) { $datos->set9($archivoimg3); };			
							$datos->update();
							$close= $datos->closeconexion();
